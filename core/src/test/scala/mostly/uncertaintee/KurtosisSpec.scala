...
class KurtosisSpec extends RngSuite {
  ...
  /**
   * Asserts that the excess kurtosis of a distribution matches the theoretical value.
   * This function uses multiple smaller runs and averages the kurtosis to get a stable estimate,
   * avoiding the high variance of a single, large sample run.
   *
   * @param distribution The Uncertain distribution to test.
   * @param theoreticalKurtosis The expected theoretical excess kurtosis.
   * @param distName A string name for the distribution for clear test failure messages.
   * @param iterations The number of runs to average over.
   * @param samplesPerIteration The number of samples to take in each run.
   */
  private def assertKurtosis(
    ...
    val hint = s"Average sample excess kurtosis for $distName ($averageKurtosis) over $iterations runs should be close to $theoreticalKurtosis."
