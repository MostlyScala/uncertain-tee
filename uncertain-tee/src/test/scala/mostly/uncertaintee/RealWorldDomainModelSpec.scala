...
class RealWorldDomainModelSpec extends RngSuite {
  ...
  rngTest("3. Finance: Projecting Profitability") {
    ...
    val probabilityOfProfit = profitability.map(_ == Profitable).probability(sampleCount)
    assert(
  ...
  rngTest("5. Project Management: Estimating Project Timelines") {
    ...
    val probabilityDelayed = timeline.map(_ == Delayed).probability(sampleCount)
  ...
  rngTest("8. Systems Engineering: Simulating Server Load Status") {
    ...
    val probabilityOverloaded = serverStatus.map(_ == Overloaded).probability(sampleCount)
